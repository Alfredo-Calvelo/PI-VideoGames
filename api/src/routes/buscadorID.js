...
router.get('/:ID', function(req,res,next){
  ...
  let cargar = respuesta => {
    respuesta.data.results.map(elemento=>{
        let{id,name,rating,background_image, genres, released,platforms} = elemento

        arr.push({id,name,released,background_image, rating, 
...
// una vez terminado ese proceso la funcion da una respuesta hacia 
  async function hacer (){
    ...
    let arrReturn = arr.filter(game => game.id === parseInt(req.params.ID,10))
